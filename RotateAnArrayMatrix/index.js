...
const rotate = (matrix, direction) => {
    ...
    const matrixM = direction === "counter-clockwise" ? matrix.slice().reverse() : matrix.slice();
    for (let i = 0; i < matrixM.length; i++) {
        const curRow =
            direction === "counter-clockwise" ? matrixM[i].slice().reverse() : matrixM[i];

        for (let j = 0; j < curRow.length; j++) {
            rotateMatrix[j][curRow.length - i - 1] = curRow[j];
        }
    }
