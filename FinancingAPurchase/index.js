function amort(rate, bal, term, num_payments) {
  let rateMonth = rate / 100 / 12;
  let c =
    bal *
    ((rateMonth * (rateMonth + 1) ** term) / ((1 + rateMonth) ** term - 1));
  ...
  return `num_payment ${num_payments.toFixed(0)} c ${c.toFixed(
    0
  )} princ ${princ.toFixed(0)} int ${int.toFixed(0)} balance ${curBal.toFixed(
    0
  )}`;
}
